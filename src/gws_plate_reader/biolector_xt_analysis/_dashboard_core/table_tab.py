...
from gws_core import File, FrontService, ResourceModel, ResourceOrigin, Settings, TableImporter, Tag
...
def render_table_tab():
    ...
    for filter_selection in selected_filters:
        ...
        if df is not None and len(df.columns) > 2:
            ...
            # Create columns for title and button on the same line
            title_col, button_save_col = StreamlitContainers.columns_with_fit_content(
                cols=[1, "fit-content"],
                vertical_align_items="center",
                key=f"container-column-{filter_selection}",
            )
            ...
            if not BiolectorState.is_standalone():
                with button_save_col:
                    ...
                    if st.button(
                        f"Save {filter_selection} table",
                        icon=":material/save:",
                        key=f"save_{filter_selection}",
                        type="primary",
                    ):
                        ...
                        resource_name = (
                            f"{st.session_state['raw_data']}_{filter_selection}"
                            if "raw_data" in st.session_state
                            and st.session_state["raw_data"] is not None
                            else filter_selection
                        )
                        ...
                        if (
                            "comment_tag" in st.session_state
                            and st.session_state["comment_tag"] is not None
                        ):
                            tab_parsed_table.tags.add_tag(
                                Tag(
                                    key="comment",
                                    value=st.session_state["comment_tag"],
                                    origins=origins,
                                )
                            )

                        if (
                            "name_tag" in st.session_state
                            and st.session_state["name_tag"] is not None
                        ):
                            tab_parsed_table.tags.add_tag(
                        ...
                        if (
                            "user_name_tag" in st.session_state
                            and st.session_state["user_name_tag"] is not None
                        ):
                            tab_parsed_table.tags.add_tag(
                                Tag(
                                    key="user_name",
                                    value=st.session_state["user_name_tag"],
                                    origins=origins,
                                )
                            )

                        if (
                            "date_tag" in st.session_state
                            and st.session_state["date_tag"] is not None
                        ):
                            tab_parsed_table.tags.add_tag(
                        ...
                        tab_parsed_table.tags.add_tag(
                            Tag(key="origin", value="biolector_dashboard", origins=origins)
                        )
                        ...
                        for col in tab_parsed_table.column_names:
                            ...
                            tab_parsed_table.add_column_tag_by_name(
                                col, key=Tag.parse_tag("well"), value=Tag.parse_tag(col)
                            )
                            if dict_col is not None:
                                for key, value in dict_col.items():
                                    tab_parsed_table.add_column_tag_by_name(
                                        col, key=Tag.parse_tag(key), value=Tag.parse_tag(value)
                                    )

                        tab_parsed_resource = ResourceModel.save_from_resource(
                            tab_parsed_table, ResourceOrigin.UPLOADED, flagged=True
                        )
                        BiolectorState.set_display_success_message_resource_table(True)

                if BiolectorState.get_display_success_message_resource_table():
                    st.success(
            ...
            with StreamlitContainers.full_width_dataframe_container(
                "container-full-dataframe-" + str(filter_selection)
            ):
