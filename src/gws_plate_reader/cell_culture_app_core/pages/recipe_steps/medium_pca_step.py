...
def launch_medium_pca_scenario(
    ...
    :param columns_to_exclude: List of columns to exclude from PCA (optional)
    ...
    try:
        with StreamlitAuthenticateUser():
            ...
            filter_task = protocol_proxy.add_process(
                CellCultureMediumTableFilter, "medium_filter_task"
            )
            ...
            pca_task = protocol_proxy.add_process(CellCultureMediumPCA, "medium_pca_task")

            # Connect the filtered table to the PCA task
            protocol_proxy.add_connector(
                out_port=filter_task >> "filtered_table", in_port=pca_task << "medium_table"
            )
            ...
            protocol_proxy.add_output(
                "pca_scores_table", pca_task >> "scores_table", flag_resource=True
            )
            protocol_proxy.add_output(
                "pca_scatter_plot", pca_task >> "scatter_plot", flag_resource=True
            )
            protocol_proxy.add_output("pca_biplot", pca_task >> "biplot", flag_resource=True)
...
def render_medium_pca_step(
    ...
    with st.form(key=f"medium_pca_form_{quality_check_scenario.id}"):
        ...
        try:
            ...
            excludable_columns = [
                col for col in available_columns if col != cell_culture_state.MEDIUM_COLUMN_NAME
            ]
            ...
            columns_to_exclude = st.multiselect(
                translate_service.translate("pca_columns_to_exclude"),
                options=excludable_columns,
                default=[],
                help=translate_service.translate("pca_columns_to_exclude_help"),
            )
