...
@task_decorator(
    ...
    human_name="Cell Culture Medium Table Filter",
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        if selected_medium and len(selected_medium) > 0:
            ...
            self.log_info_message(
                f"Filtered to {len(filtered_df)} rows matching {len(found_medium)} selected medium"
            )
        else:
            # No filter, keep all
            filtered_df = df.copy()
        ...
        filtered_table.name = (
            f"{medium_table.name} (filtered)"
            if hasattr(medium_table, "name")
            else "Filtered Medium Table"
        )
