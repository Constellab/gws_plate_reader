"""
Spline Growth Rate Analysis Step for Cell Culture Dashboard
Allows users to run spline-based growth rate inference with cross-validation
"""
...
from gws_plate_reader.features_extraction.spline_growth_rate_inference import SplineGrowthRateInference
from gws_plate_reader.fermentalg_analysis import ResourceSetToDataTable


def launch_spline_growth_scenario(
        ...
        s_min: float = 0.01,
        s_max: float = 100.0,
        n_s_values: int = 500) -> Optional[Scenario]:
    """
    Launch a Spline Growth Rate Inference scenario
    ...
    :param s_min: Minimum smoothing parameter to test
    :param s_max: Maximum smoothing parameter to test
    :param n_s_values: Number of smoothing values to test
    ...
    try:
        with StreamlitAuthenticateUser():
            ...
            scenario_title = f"Spline Growth - {data_column} - {timestamp}"
            ...
            # Add Spline Growth Rate Inference task
            spline_inference_task = protocol_proxy.add_process(
                SplineGrowthRateInference,
                'spline_growth_rate_inference'
            )

            # Connect table to inference task
            protocol_proxy.add_connector(
                out_port=rs_to_table_task >> 'data_table',
                in_port=spline_inference_task << 'table'
            )

            # Set parameters
            spline_inference_task.set_param('n_splits', n_splits)
            spline_inference_task.set_param('s_min', s_min)
            spline_inference_task.set_param('s_max', s_max)
            spline_inference_task.set_param('n_s_values', n_s_values)

            # Add outputs
            protocol_proxy.add_output(
                'parameters',
                spline_inference_task >> 'parameters',
                flag_resource=True
            )
            protocol_proxy.add_output(
                'growth_curves_plot',
                spline_inference_task >> 'growth_curves_plot',
                flag_resource=True
            )
            protocol_proxy.add_output(
                'growth_rate_comparison',
                spline_inference_task >> 'growth_rate_comparison',
                flag_resource=True
            )
            ...
            scenario_proxy.add_tag(Tag("analysis_type", "spline_growth_rate", is_propagable=False))
        ...
        st.error(f"Erreur lors du lancement du scénario Spline Growth Rate: {str(e)}")
        import traceback
        st.code(traceback.format_exc())
        return None


def render_spline_growth_step(recipe: CellCultureRecipe, cell_culture_state: CellCultureState,
                              quality_check_scenario: Scenario) -> None:
    """
    Render the Spline Growth Rate Inference step
    ...
    if spline_scenarios:
        for scenario in spline_scenarios:
