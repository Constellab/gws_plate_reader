...
def show_content():
    ...
    analysis_page = st.Page(
        render_analysis_page, title="Growth rate analysis", url_path="protocols", icon="🔍"
    )
...
def run(data: dict[str, Any], is_standalone: bool, input_tag: Tag):
    BiolectorState.init(
        data=data,
        mode=BiolectorStateMode.SINGLE_PLATE
        if not is_standalone
        else BiolectorStateMode.STANDALONE,
        input_tag=input_tag,
    )
    ...
    with st.sidebar:
        ...
        def fragment_sidebar_function():
            ...
            for _well, info in well_data.items():
                all_keys_well_description.update(info.keys())
            all_keys_well_description = list(all_keys_well_description)

            # Color microplate by ...
            selected_color_well = st.selectbox(
                "Color microplate by ...",
                all_keys_well_description,
                index=0,
                key="selected_color_well",
            )
            ...
            unique_labels = list(
                {
                    re.sub(
                        ...
                        unicodedata.normalize(
                            "NFKD", well.get(selected_color_well, "").strip().replace(" ", "_")
                        )
                        .encode("ascii", "ignore")
                        .decode("utf-8"),
                    )
            ...
            # Define the structure of the 48-well microplate
            wells = [
                [f"{chr(65 + row)}{col + 1:02d}" for col in range(COLS)] for row in range(ROWS)
            ]
            ...
            for row in range(ROWS):
                ...
                for col in range(COLS):
                    ...
                    label_normalized = re.sub(
                        ...
                        unicodedata.normalize(
                            "NFKD",
                            well_data[well].get(selected_color_well, "").strip().replace(" ", "_"),
                        )
                        .encode("ascii", "ignore")
                        .decode("utf-8"),
                    )
                    ...
                    if well in crossed_out_wells:
                        cols_object[col + 1].button(
                            f":gray[{well}]", key=key, help=help_tab, disabled=True
                        )
