...
@task_decorator(
    ...
    human_name="BiolectorXT Data Parser",
...
class BiolectorXTDataParser(Task):
    ...
    output_specs: OutputSpecs = OutputSpecs(
        {
            "parsed_data_tables": OutputSpec(
                ResourceSet, human_name="Parsed data tables resource set", sub_class=True
            )
        }
    )
    ...
    def parse_data(self, data: DataFrame, metadata: dict) -> dict[str, DataFrame]:
        ...
        for i, value in enumerate(unique_values):
            ...
            df_filter = df_filter.assign(
                time=NA, Temps_en_h=NA, **dict.fromkeys(columns_to_add, NA)
            )
    ...
    def create_parsed_resource_set(
        self, data: DataFrame, metadata: dict, existing_plate_layout: dict | None = None
    ) -> ResourceSet:
        """
        ...
        for well, well_data in wells_data.items():
            for table_key in parsed_data_tables:
                if well in parsed_data_tables[table_key].get_data().columns:
                    for data_key, data_value in well_data.items():
                        parsed_data_tables[table_key].add_column_tag_by_name(
                            well, data_key, data_value
                        )
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        resource_set = self.create_parsed_resource_set(
            data=raw_data.get_data(), metadata=metadata, existing_plate_layout=existing_plate_layout
        )
        ...
        if name:
            list_tags.append(
                Tag(key="name", value=name, auto_parse=True, origins=origins, is_propagable=True)
            )
        ...
        if date:
            list_tags.append(
                Tag(key="date", value=date, auto_parse=True, origins=origins, is_propagable=True)
            )

        list_tags.append(
            Tag(
                key="raw_data",
                value=raw_data.name.lower().replace(" ", "_"),
                origins=origins,
                is_propagable=True,
            )
        )
        ...
        for _table_name, table in resource_set.get_resources().items():
