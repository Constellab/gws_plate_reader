...
def render_new_recipe_microplate(cell_culture_state: CellCultureState) -> None:
    ...
    if file_input_mode == 'upload':
        ...
        medium_table_resource = st.session_state.get("microplate_medium_table_selector").get(
            "resourceId", None) if st.session_state.get("microplate_medium_table_selector") else None
    ...
    for plate_idx in st.session_state.microplate_plates_list:
        # Plate title
        st.write(f"### 🧫 {translate_service.translate('plate')} {st.session_state.microplate_plates_list.index(plate_idx) + 1}")
        ...
        with st.container():
            ...
            if file_input_mode == 'upload':
                ...
                for key in [f"microplate_raw_data_selector_{plate_idx}", f"microplate_folder_metadata_selector_{plate_idx}", f"microplate_info_table_selector_{plate_idx}"]:
                    if key not in st.session_state:
                        st.session_state[key] = {}

                raw_data_resource = st.session_state.get(f"microplate_raw_data_selector_{plate_idx}").get(
                    "resourceId", None) if st.session_state.get(f"microplate_raw_data_selector_{plate_idx}") else None
                folder_metadata_resource = st.session_state.get(f"microplate_folder_metadata_selector_{plate_idx}").get(
                    "resourceId", None) if st.session_state.get(f"microplate_folder_metadata_selector_{plate_idx}") else None
                info_table_resource = st.session_state.get(f"microplate_info_table_selector_{plate_idx}").get(
                    "resourceId", None) if st.session_state.get(f"microplate_info_table_selector_{plate_idx}") else None
    ...
    # Add plate button
    if st.button(f"➕ {translate_service.translate('add_plate')}", use_container_width=True, type="secondary", key="microplate_add_plate_button"):
    ...
    if st.button(
        ...
        try:
            ...
            scenario.add_tag(Tag(cell_culture_state.TAG_FERMENTOR,
                                cell_culture_state.TAG_DATA_PROCESSING,
                                is_propagable=False))
            scenario.add_tag(Tag(cell_culture_state.TAG_FERMENTOR_RECIPE_NAME,
                                analysis_name_parsed,
                                is_propagable=False))
            scenario.add_tag(Tag(cell_culture_state.TAG_FERMENTOR_PIPELINE_ID,
                                pipeline_id,
                                is_propagable=False))
            scenario.add_tag(Tag(cell_culture_state.TAG_MICROPLATE_ANALYSIS,
                                "true",
                                is_propagable=False))
