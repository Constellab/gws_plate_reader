...
class LogisticGrowthFitter:
    ...
    def fit_logistic_growth_with_cv(self):
        ...
        for well in self.data.columns[1:]:
            ...
            if plate_name:
                label = (
                    dict_well_data_description.get(display_well, {})
                    .get(plate_name, {})
                    .get("label", None)
                )
            else:
            ...
            for train_index, test_index in kf.split(time):
                ...
                well_data_pred = self.logistic_growth(
            ...
            (
                fitted_max_absorbance,
                fitted_growth_rate,
                fitted_lag_time,
                fitted_initial_absorbance,
            ) = best_fit_params
            params_dict = {
                "Well": [display_well],
                "Label": [label],
            ...
            curves_dict = {"Time": time_fitted, "Fitted_Curve": fitted_curve, "Well": display_well}
        ...
        self.df_fitted_curves = pd.concat(df_fitted_curves_list, ignore_index=True).set_index(
            "Well"
        )
    ...
    def plot_fitted_curves_with_r2(self):
        ...
        for i, well in enumerate(wells):
            ...
            if plate_name and "Plate_Name" in self.df_params.columns:
                ...
                label = (
                    reduced_plate_name.loc[display_well, "Label"]
                    if "Label" in reduced_plate_name.columns
                    else None
                )
            else:
                label = (
                    self.df_params.loc[display_well, "Label"]
                    if "Label" in self.df_params.columns
                    else None
                )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    marker={"color": color, "symbol": "circle", "size": 6},
                    showlegend=True,
                )
            )
    ...
    def plot_growth_rate_histogram(self):
        ...
        fig.add_trace(
            go.Histogram(
                ...
                hoverinfo="x+y",
                xbins={"start": growth_rates.min(), "end": growth_rates.max(), "size": bin_size},
            )
        )
