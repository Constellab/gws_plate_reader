...
def show_content():
    ...
    with tab_dict:
        ...
        length_diff = len(st.session_state.compounds) - len(st.session_state.dilutions)
        ...
        edited_dict_df = st.data_editor(dict_df, use_container_width = True, hide_index=True, num_rows="dynamic")
        ...
        if st.button("Save data") :
            ...
            file_dict_compounds_path = os.path.join(folder_data,"compounds.json")
            with open(file_dict_compounds_path, "w") as json_file:
                json.dump(st.session_state.compounds, json_file, indent=4)

            file_dict_dilutions_path = os.path.join(folder_data,"dilutions.json")
...
files_compounds = [f for f in os.listdir(folder_data) if f.endswith("compounds.json")]
if files_compounds :
...
files_dilutions = [f for f in os.listdir(folder_data) if f.endswith("dilutions.json")]
if files_dilutions :
...
files_plate_layout = [f for f in os.listdir(folder_data) if f.endswith("plate_layout.json")]
