...
from gws_core import InputTask, Scenario, ScenarioCreationType, ScenarioProxy, Table, Tag
...
def launch_selection_scenario(
    ...
    try:
        ...
        resource_set_model = (
            ScenarioProxy.from_existing_scenario(load_scenario.id)
            .get_protocol()
            .get_process(cell_culture_state.PROCESS_NAME_DATA_PROCESSING)
            .get_output_resource_model("resource_set")
        )
        ...
        input_task = protocol_proxy.add_process(
            InputTask, "resource_set_input", {InputTask.config_name: resource_set_model.id}
        )

        # Add the filter task
        filter_task = protocol_proxy.add_process(
            FilterFermentorAnalyseLoadedResourceSetBySelection, "filter_selection_task"
        )

        # Add the subsampling task
        subsampling_task = protocol_proxy.add_process(CellCultureSubsampling, "subsampling_task")

        # Connect the ResourceSet to the filter task
        protocol_proxy.add_connector(
            out_port=input_task >> "resource", in_port=filter_task << "resource_set"
        )
        ...
        for _, row in selected_df.iterrows():
            selection_criteria.append({"batch": row["Batch"], "sample": row["Sample"]})
        ...
        if (
            hasattr(cell_culture_state, "BASE_TIME_COLUMN_NAME")
            and cell_culture_state.BASE_TIME_COLUMN_NAME
        ):
            interpolation_config["time_column"] = cell_culture_state.BASE_TIME_COLUMN_NAME
        if (
            hasattr(cell_culture_state, "BATCH_COLUMN_NAME")
            and cell_culture_state.BATCH_COLUMN_NAME
        ):
            interpolation_config["batch_column"] = cell_culture_state.BATCH_COLUMN_NAME
        if (
            hasattr(cell_culture_state, "SAMPLE_COLUMN_NAME")
            and cell_culture_state.SAMPLE_COLUMN_NAME
        ):
        ...
        original_recipe_name = (
            parent_recipe_name_tags[0].tag_value if parent_recipe_name_tags else load_scenario.title
        )
        ...
        pipeline_id = (
            parent_pipeline_id_tags[0].tag_value if parent_pipeline_id_tags else load_scenario.id
        )
...
def render_selection_step(recipe: CellCultureRecipe, cell_culture_state: CellCultureState) -> None:
    ...
    try:
        ...
        if valid_data:
            ...
            st.write(
                f"**{translate_service.translate('valid_samples_available').format(count=len(df_valid))}**"
            )
            ...
            if st.button(
                ...
                if selected_data.selection.rows:
                    ...
                    try:
                        ...
                        st.error(
                            translate_service.translate("error_launching_scenario_short").format(
                                error=str(e)
                            )
                        )
                    ...
                    st.write(
                        "**"
                        + translate_service.translate("samples_selected").format(
