...
def render_table_tab():
    ...
    for filter_selection in selected_filters:
        ...
        if df is not None and len(df.columns) > 2:
            ...
            title_col, button_save_col = StreamlitContainers.columns_with_fit_content(cols=[1, 'fit-content'],
                vertical_align_items='center', key =f'container-column-{filter_selection}')
            ...
            if not BiolectorState.is_standalone():
                with button_save_col:
                    ...
                    if st.button(f"Save {filter_selection} table", icon=":material/save:", key=f"save_{filter_selection}", type="primary"):
