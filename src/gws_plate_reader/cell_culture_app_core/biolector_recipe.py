...
_TAG_BIOLECTOR_RECIPE_NAME = "biolector_recipe_name"
_TAG_BIOLECTOR_PIPELINE_ID = "biolector_pipeline_id"
_TAG_BIOLECTOR_QUALITY_CHECK_PARENT_SELECTION = "biolector_quality_check_parent_selection"
_TAG_BIOLECTOR_ANALYSES_PARENT_SELECTION = "biolector_analyses_parent_selection"
_TAG_BIOLECTOR_ANALYSES_PARENT_QUALITY_CHECK = "biolector_analyses_parent_quality_check"
...
class BiolectorRecipe(CellCultureRecipe):
    ...
    def from_scenario(cls, scenario: Scenario) -> 'BiolectorRecipe':
        ...
        name = cls._extract_tag_value(
            entity_tag_list, _TAG_BIOLECTOR_RECIPE_NAME, scenario.title)

        # Extract pipeline ID using BiolectorXT-specific tag
        pipeline_id = cls._extract_tag_value(
            entity_tag_list, _TAG_BIOLECTOR_PIPELINE_ID, scenario.id)
    ...
    def get_quality_check_scenarios_for_selection(self, selection_id: str) -> list[Scenario]:
        ...
        for scenario in all_qc_scenarios:
            ...
            parent_selection_tags = entity_tag_list.get_tags_by_key(
                _TAG_BIOLECTOR_QUALITY_CHECK_PARENT_SELECTION)
    ...
    def get_analyses_scenarios_for_selection(self, selection_id: str) -> list[Scenario]:
        ...
        for scenario in all_analyses_scenarios:
            entity_tag_list = EntityTagList.find_by_entity(TagEntityType.SCENARIO, scenario.id)
            parent_selection_tags = entity_tag_list.get_tags_by_key(
                _TAG_BIOLECTOR_ANALYSES_PARENT_SELECTION)
            if parent_selection_tags and parent_selection_tags[0].tag_value == selection_id:
                filtered_scenarios.append(scenario)
        return filtered_scenarios
    ...
    def get_medium_pca_scenarios_for_quality_check(self, qc_id: str) -> list[Scenario]:
        ...
        for scenario in all_analyses_scenarios:
            ...
            parent_qc_tags = entity_tag_list.get_tags_by_key(
                _TAG_BIOLECTOR_ANALYSES_PARENT_QUALITY_CHECK)
