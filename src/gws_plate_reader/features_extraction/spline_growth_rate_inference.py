...
@task_decorator(
    ...
    human_name="Spline Growth Rate Inference",
...
class SplineGrowthRateInference(Task):
    ...
    **non-parametric spline smoothing** with cross-validation.
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        # Perform inference
        df_params, spline_data = self._infer_growth_rates(df, s_values, n_splits)
    ...
    def _infer_growth_rates(self, data: pd.DataFrame, s_values: np.ndarray, n_splits: int):
            ...
            for s in s_values:
                ...
                for train_idx, val_idx in kf.split(time_valid):
                    ...
                    absorbance_train, absorbance_val = (
                        absorbance_valid[train_idx],
                        absorbance_valid[val_idx],
                    )
        ...
        if len(df_params_list) == 0:
            raise ValueError(
                "Could not analyze any wells. Please check your data for sufficient valid (non-NaN) points."
            )
    ...
    def _plot_growth_curves(self, data: pd.DataFrame, spline_data: dict) -> PlotlyResource:
        ...
        for i, well in enumerate(wells):
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers",
                    name=f"{well} - Raw",
                    marker={"color": color, "symbol": "circle", "size": 4, "opacity": 0.5},
                    legendgroup=well,
                    showlegend=True,
                ),
                row=1,
        ...
        fig.update_layout(title="Spline-Based Growth Analysis", template="plotly_white", height=500)
