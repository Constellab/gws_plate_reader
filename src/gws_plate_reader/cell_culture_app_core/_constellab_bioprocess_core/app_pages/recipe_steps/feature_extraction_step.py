...
def get_available_columns_from_quality_check(
    quality_check_scenario: Scenario, cell_culture_state: CellCultureState, index_only: bool = False
) -> list[str]:
    """
    ...
    try:
        ...
        resource_set_resource_model = (
            cell_culture_state.get_quality_check_scenario_output_resource_model(
                quality_check_scenario
            )
        )
        resource_set = (
            resource_set_resource_model.get_resource() if resource_set_resource_model else None
        )
...
def launch_feature_extraction_scenario(
    ...
    try:
        ...
        resource_set_input_task = protocol_proxy.add_process(
            InputTask, "resource_set_input", {InputTask.config_name: qc_output.id}
        )

        # Add ResourceSetToDataTable task
        rs_to_table_task = protocol_proxy.add_process(
            ResourceSetToDataTable, "resource_set_to_table"
        )
        ...
        feature_extraction_task = protocol_proxy.add_process(
            CellCultureFeatureExtraction, "feature_extraction_task"
        )
        ...
        protocol_proxy.add_output(
            "results_table", feature_extraction_task >> "results_table", flag_resource=True
        )
        protocol_proxy.add_output("plots", feature_extraction_task >> "plots", flag_resource=True)
...
def render_feature_extraction_step(
    ...
    with st.form(key=f"feature_extraction_form_{quality_check_scenario.id}"):
        ...
        if submit_button:
            if not selected_data_columns or not selected_models:
                ...
                for idx, data_column in enumerate(selected_data_columns):
                    ...
                    progress_bar.progress(
                        progress,
                        text=f"{translate_service.translate('launching')} {data_column}...",
                    )
                    ...
                    if fe_scenario:
                        ...
                        # Add to recipe
                        recipe.add_feature_extraction_scenario(
                            quality_check_scenario.id, fe_scenario
                        )
