...
def launch_pls_regression_scenario(
    ...
    try:
        ...
        st.error(f"Erreur lors du lancement du scénario PLS Regression: {str(e)}")
        import traceback
...
def render_pls_regression_step(recipe: CellCultureRecipe, cell_culture_state: CellCultureState,
    ...
    st.markdown(f"### 📊 Analyse PLS Regression")

    st.info("La régression PLS (Partial Least Squares) combine les données de composition des milieux avec les caractéristiques biologiques pour identifier les relations entre composition et performances de croissance.")
    ...
    if not load_scenario:
        st.warning("⚠️ Aucun scénario de chargement de données trouvé. L'analyse PLS n'est pas disponible.")
        return
    ...
    if existing_pls_scenarios:
        st.markdown(f"**Analyses PLS existantes** : {len(existing_pls_scenarios)}")
        with st.expander("📊 Voir les analyses PLS existantes"):
    ...
    with col1:
        scale_data = st.checkbox(
            ...
            help="Standardiser les données avant PLS (fortement recommandé)"
        )
    ...
    columns_to_exclude = st.multiselect(
        "Colonnes à exclure de l'analyse PLS",
        ...
        help="Sélectionnez les colonnes à exclure de l'analyse. Par défaut : colonnes non-numériques et colonnes de feature extraction (sauf cibles)."
    )
    ...
    if st.button(
        ...
        if len(target_columns) == 0:
            ...
            if pls_scenario:
                st.success(f"✅ Analyse PLS lancée avec succès ! ID : {pls_scenario.id}")
                ...
                st.error("❌ Erreur lors du lancement de l'analyse PLS")

    # Info box with explanation
    with st.expander("💡 Aide sur l'analyse PLS Regression"):
        st.markdown("""
