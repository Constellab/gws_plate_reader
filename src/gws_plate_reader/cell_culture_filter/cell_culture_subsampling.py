...
class CellCultureSubsampling(Task):
    ...
    output_specs: OutputSpecs = OutputSpecs(
        {
            "subsampled_resource_set": OutputSpec(
                ResourceSet,
    ...
    def auto_n_points_from_time_list(
        self, t_list: list[np.ndarray], max_points=20000, min_points=100
    ) -> int:
    ...
    def build_global_grid(
        self, dfs: list[pd.DataFrame], time_col: str, n_points: int | None = None
    ) -> np.ndarray:
    ...
    def _dedup_time_average(
        self, t_valid: np.ndarray, y_valid: np.ndarray
        ...
        for i, yi in zip(inv, y_valid, strict=False):
            y_accum[i] += yi
            counts[i] += 1
        ...
        if method == "univariate_spline":
            ...
            try:
                ...
                self.log_warning_message(
                    "UnivariateSpline failed, falling back to linear interpolation"
                )
                return np.interp(t_new, t_valid, y_valid)
    ...
    def interpolate_one(
        ...
        min_values_threshold: int | None = None,
    ) -> pd.DataFrame:
        ...
        for col in numeric_cols:
            ...
            if min_values_threshold is not None and t_valid.size < min_values_threshold:
            ...
            y_core = self._core_interpolate(
                t_valid, y_valid, t_new, method=method, spline_order=spline_order
            )
    ...
    def run(self, params: ConfigParams, inputs) -> dict[str, Any]:
        ...
        self.log_info_message(
            f"Starting interpolation with method: {method}, grid strategy: {grid_strategy}"
        )
        if min_values_threshold is not None:
            self.log_info_message(
                f"Only interpolating columns with at least {min_values_threshold} non-NaN values"
            )
        ...
        for resource_name, resource in resources.items():
            ...
            metadata[resource_name] = {
                ...
                "tags": resource.tags,
                "column_tags": {
                    col: resource.get_column_tags_by_name(col)
                    for col in resource.get_column_names()
                },
            }
        ...
        if grid_strategy == "per_file":
            ...
            for name, df in raw_dfs.items():
                ...
                t_new_local = np.linspace(
                    float(np.nanmin(t_array)), float(np.nanmax(t_array)), int(local_n)
                )
        ...
        for resource_name, interpolated_df in results.items():
            ...
            combined_times = (
                pd.concat([original_df[time_col], interpolated_df_with_time[time_col]])
                .drop_duplicates()
                .sort_values()
            ...
            for col in all_columns:
                if col in interpolated_columns:
                    ...
                    temp_df = combined_df.merge(
                        interpolated_df_with_time[[time_col, col]], on=time_col, how="left"
                    )
                    ...
                    temp_df = combined_df.merge(
                        original_df[[time_col, col]], on=time_col, how="left"
                    )
                    combined_df[col] = temp_df[col]
        ...
        self.log_success_message(
            f"Successfully subsampled {len(results)} resources using {method} method"
        )

        return {"subsampled_resource_set": subsampled_res}
