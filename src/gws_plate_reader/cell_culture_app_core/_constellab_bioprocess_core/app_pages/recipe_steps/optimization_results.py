...
def render_optimization_results(
    cell_culture_state: CellCultureState, optimization_scenario: Scenario
) -> None:
    """
    ...
    st.markdown(
        "**" + translate_service.translate("scenario_label") + "** : " + optimization_scenario.title
    )
    ...
    if status == ScenarioStatus.SUCCESS:
        ...
        st.error(
            f"❌ **{translate_service.translate('status_label')}** : {translate_service.translate('status_error_execution')}"
        )
        ...
        try:
            ...
            if protocol_proxy:
                ...
                st.code(
                    protocol_proxy.get_error_message()
                    if hasattr(protocol_proxy, "get_error_message")
                    else translate_service.translate("unknown_error")
                )
        except Exception as e:
            st.warning(
                translate_service.translate("unable_retrieve_error_details").format(error=str(e))
            )
