...
from gws_core import File, FrontService, ResourceModel, ResourceOrigin, Settings, TableImporter
    ...
    with st.spinner("Running analysis..."):
        ...
        try:
            ...
            if not BiolectorState.is_standalone():
                empty_col, button_save_col = StreamlitContainers.columns_with_fit_content(
                    cols=[1, "fit-content"],
                    vertical_align_items="center",
                    key=f"container-column-analysis",
                )
                ...
                with button_save_col:
                    if st.button("Save analysis table", icon=":material/save:", type="primary"):
                        ...
                        analysis_df_table = TableImporter.call(
                            analysis_df, params={"index_column": 0}
                        )
            ...
            with StreamlitContainers.full_width_dataframe_container(
                ...
                st.dataframe(df_analysis.style.format(thousands=" ", precision=4), width="stretch")
