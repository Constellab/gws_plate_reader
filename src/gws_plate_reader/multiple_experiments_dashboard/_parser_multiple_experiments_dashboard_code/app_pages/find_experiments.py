...
from gws_streamlit_main import StreamlitContainers, StreamlitResourceSelect


def add_experiment_to_experiments_list(
    r: ResourceModel, table: Table
) -> Dict[str, "BiolectorExperimentWithDetails"]:
    experiments_list: Dict[str, BiolectorExperimentWithDetails] = st.session_state.get(
        "experiments_list", {}
    )

    list_raw_data_id_biolector_experiment = [
        (e.raw_data, e.id_biolector_experiment) for e in experiments_list.values()
    ]
    if table is not None:
        ...
        if "-" in raw_data:
            raise ValueError(
                f"Raw data {raw_data} contains '-' character. Please rename the raw data."
            )
        ...
        num_raw_data = len(
            [i for i in list_raw_data_id_biolector_experiment if i[0].split("-")[0] == raw_data]
        )
        ...
        experiment = BiolectorExperimentWithDetails(table, raw_data, id_biolector_experiment)
...
def render_find_experiments_page():
    ...
    selected_resource = resource_select.select_resource(
        placeholder="Search and add experiment data", key="resource-selector", default_resource=None
    )
    ...
    if selected_resource:
        ...
        if (
            table.get_typing_name() != "RESOURCE.gws_core.Table"
            or len(table.tags.get_by_key("raw_data")) != 1
            or table.tags.get_tag("origin", "biolector_dashboard") is None
        ):
    ...
    if len(selected_resources) > 0:
        ...
        experiments_list_dict = [
            experiment.to_detail_dict() for experiment in experiments_list.values()
        ]

        if st.session_state.get("experiments_list_df", None) is None or not pd.DataFrame(
            experiments_list_dict
        ).equals(st.session_state["experiments_list_df"]):
        ...
        selected_expermiments_indexes = experiments_list_df.index[
            experiments_list_df["selected"]
        ].tolist()
        ...
        for experiment_id, experiment in experiments_list.items():
            count_selected_experiments_matching = len(
                selected_experiments_df[
                    (
                        selected_experiments_df["id_biolector_experiment"]
                        == experiment.id_biolector_experiment
                    )
        ...
        if st.session_state.get(
            "experiments_list_df", None
        ) is None or not experiments_list_df.equals(st.session_state["experiments_list_df"]):
        ...
        if st.button(
            ...
            key="add_selected_experiments_button",
            disabled=(
                len(selected_experiments.keys()) == 0
                or (
                    st.session_state.get("selected_experiments", None) is not None
                    and st.session_state["selected_experiments"].keys()
