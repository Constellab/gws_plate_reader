...
def render_quality_check_config_form(
    ...
    with st.expander(f"📊 {translate_service.translate('qc_range_checks_title')}", expanded=False):
        ...
        for i in range(int(num_range_checks)):
            ...
            range_checks.append(
                {"column": col_name, "min_value": min_val, "max_value": max_val, "action": action}
            )
    ...
    with st.expander(
        ...
        for i in range(int(num_min_points_checks)):
            ...
            min_data_points.append(
                {"column": col_name, "min_count": float(min_count), "action": action}
            )
...
def launch_quality_check_scenario(
    ...
    try:
        ...
        with StreamlitAuthenticateUser():
            ...
            subsampled_resource = cell_culture_state.get_interpolation_scenario_output(
                selection_scenario
            )
            ...
            data_input_task = protocol_proxy.add_process(
                InputTask, "data_input", {InputTask.config_name: data_resource_id}
            )

            # Add input task for the subsampled ResourceSet (for visualizations)
            subsampled_input_task = protocol_proxy.add_process(
                InputTask, "subsampled_input", {InputTask.config_name: subsampled_resource_id}
            )
            ...
            quality_check_task = protocol_proxy.add_process(
                CellCultureQualityCheck, "quality_check_task"
            )

            # Connect the real data ResourceSet to the quality check task
            protocol_proxy.add_connector(
                out_port=data_input_task >> "resource", in_port=quality_check_task << "data"
            )
            ...
            parent_entity_tag_list = EntityTagList.find_by_entity(
                TagEntityType.SCENARIO, selection_scenario.id
            )
            ...
            scenario_proxy.add_tag(
                Tag(
                    ...
                    cell_culture_state.TAG_QUALITY_CHECK_PROCESSING,
                    is_propagable=False,
                )
            )
            ...
            scenario_proxy.add_tag(
                Tag(
                    cell_culture_state.TAG_FERMENTOR_QUALITY_CHECK_PARENT_SELECTION,
                    selection_scenario.id,
                    is_propagable=False,
                )
            )
            scenario_proxy.add_tag(
                Tag("parent_selection_id", selection_scenario.id, is_propagable=False)
            )
...
def _render_selection_quality_checks(
    ...
    def render_content():
        ...
        quality_check_config = render_quality_check_config_form(
            selection, cell_culture_state, config_key
        )
        ...
        if st.button(
            ...
            try:
                ...
                st.error(
                    translate_service.translate("error_creating_scenario")
    ...
    if show_in_expander:
        with st.expander(
            f"**{idx + 1}. {selection.title}** - ID: {selection.id} - {qc_count} {translate_service.translate('qc_quality_checks')}"
        ):
            render_content()
    else:
        render_content()
