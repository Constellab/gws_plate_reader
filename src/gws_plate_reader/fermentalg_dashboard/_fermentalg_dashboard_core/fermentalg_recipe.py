...
class FermentalgRecipe(CellCultureRecipe):
    ...
    def get_logistic_growth_scenarios_for_quality_check(self, qc_id: str) -> List[Scenario]:
        """
        Get Logistic Growth Fitting scenarios for a specific quality check

        :param qc_id: Quality check scenario ID
        :return: List of Logistic Growth scenarios for this quality check
        """
        # Get all analyses scenarios
        all_analyses_scenarios = self.get_scenarios_for_step('analyses')
        ...
        for scenario in all_analyses_scenarios:
            ...
            is_logistic_growth = analysis_type_tags and analysis_type_tags[0].tag_value == "logistic_growth"
            is_for_qc = parent_qc_tags and parent_qc_tags[0].tag_value == qc_id
    ...
    def get_spline_growth_scenarios_for_quality_check(self, qc_id: str) -> List[Scenario]:
        """
        Get Spline Growth Rate Inference scenarios for a specific quality check

        :param qc_id: Quality check scenario ID
        :return: List of Spline Growth scenarios for this quality check
        """
        # Get all analyses scenarios
        all_analyses_scenarios = self.get_scenarios_for_step('analyses')
        ...
        for scenario in all_analyses_scenarios:
            ...
            # Check if this is a spline_growth_rate analysis for the specified quality check
            is_spline_growth = analysis_type_tags and analysis_type_tags[0].tag_value == "spline_growth_rate"
            is_for_qc = parent_qc_tags and parent_qc_tags[0].tag_value == qc_id

            if is_spline_growth and is_for_qc:
                filtered.append(scenario)
