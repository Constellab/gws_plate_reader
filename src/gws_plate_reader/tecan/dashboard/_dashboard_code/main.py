...
def show_content(microplate_object: TecanParser):
    ...
    with tab_table:
        ...
        if st.button("Update row data", use_container_width=False, icon = ":material/save:"):
...
if os.listdir(raw_data_folder):
    raw_data = pd.read_csv(os.path.join(raw_data_folder,os.listdir(raw_data_folder)[-1]), index_col= 0)
...
st.markdown(
    ...
    <style>
        section[data-testid="stSidebar"] {
            width: 530px !important; # Set the width to your desired value
        }
    </style>
    """,
