...
def render_plot_tab():
    ...
    with col1:
        ...
        selected_time = st.selectbox(
            "Select the time unit", options_time, index=index_init_value_time, key="plot_time"
        )
    ...
    with col2:
        ...
        display_mode_selected = st.selectbox(
            "Select the display mode", options_mode, index=index_init_value, key="plot_mode"
        )
    ...
    for i, filter_name in enumerate(selected_filters):
        df = BiolectorState.get_table_by_filter(
            selected_well_or_replicate, filter_name, selected_replicates
        )
        df = df.iloc[:, 1:]
        ...
        # Mean curves
            ...
            if selected_well_or_replicate == "Individual wells":
                ...
                if selected_replicates:
                    ...
                    def calculate_replicates_mean_std(df, selected_replicates, operation):
                        ...
                        for replicate in selected_replicates:
                            ...
                            if BiolectorState.get_mode() == BiolectorStateMode.MULTIPLE_PLATES:
                                for plate, wells in BiolectorState.group_wells_by_options(
                                    selected_well_or_replicate
                                )[replicate].items():
                                    for well in wells:
                                ...
                                columns = [
                                    ...
                                    if col
                                    in BiolectorState.group_wells_by_options(
                                        selected_well_or_replicate
                                    )[replicate]
                                ]
                            if operation == "mean":
                    ...
                    replicate_df_mean = calculate_replicates_mean_std(
                        df, selected_replicates, "mean"
                    )
        ...
        fig.update_layout(
            {
                f"yaxis{i + 1}": dict(
                    title=dict(
                        text=f"{filter_name}",
                        font=dict(color=f"hsl({i * 60}, 70%, 50%)"),
                        standoff=8,  # Bring the title closer to the axis
                    ),
                    tickfont=dict(color=f"hsl({i * 60}, 70%, 50%)"),
