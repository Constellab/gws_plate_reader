...
from gws_core import (
    ...
    ProcessProxy, ScenarioProxy, ProtocolProxy, ScenarioCreationType, File)
...
def render_new_recipe_page(cell_culture_state: CellCultureState) -> None:
    ...
    with StreamlitContainers.container_full_min_height('container-center_new_analysis_page',
        ...
        if file_input_mode == 'upload':
            ...
            info_csv_resource = st.session_state.get("info_csv_selector").get(
                "resourceId", None) if st.session_state.get("info_csv_selector") else None
            raw_data_csv_resource = st.session_state.get("raw_data_csv_selector").get(
                "resourceId", None) if st.session_state.get("raw_data_csv_selector") else None
            medium_csv_resource = st.session_state.get("medium_csv_selector").get(
                "resourceId", None) if st.session_state.get("medium_csv_selector") else None
            followup_zip_resource = st.session_state.get("followup_zip_selector").get(
                "resourceId", None) if st.session_state.get("followup_zip_selector") else None
        ...
        if submit_button:
            ...
            try:
                ...
                medium_csv_input = protocol.add_process(
                    InputTask, cell_culture_state.MEDIUM_CSV_INPUT_KEY,
                ...
                follow_up_zip_input = protocol.add_process(
                    ...
                    {InputTask.config_name: file_resource_models['followup_zip'].id})                    # Add the data loading task
                ...
                scenario.add_tag(Tag("raw_data_csv_file", Tag.parse_tag(
                    file_resource_models['raw_data_csv'].name), is_propagable=False))
