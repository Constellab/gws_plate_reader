...
@task_decorator(
    ...
    human_name="Cell Culture Data Quality Check",
    short_description="Apply quality checks and filters to cell culture data (outliers, ranges, missing values)",
    style=TypingStyle.material_icon(material_icon_name="check-circle", background_color="#ffc107"),
)
class CellCultureQualityCheck(Task):
    """
    ...
    ## Overview
    This task performs data quality validation and filtering on cell culture ResourceSets.
    ...
    CellCultureLoadData or FilterCellCultureAnalyseLoadedResourceSetBySelection.
    ...
    **Example Ranges:**
    ...
    ### data (ResourceSet)
    - **Source**: CellCultureLoadData or Filter task output (raw data)
    - **Requirements**:
    ...
    **Use Case**: Ensure sufficient measurements for growth curve fitting (needs ≥3 points)
    ...
    - Compatible with all CellCulture workflow tasks
    ...
    input_specs: InputSpecs = InputSpecs(
        {
            "data": InputSpec(
                ...
                short_description="ResourceSet containing cell culture time series data to validate",
                optional=False,
            ),
            "subsampled_data": InputSpec(
                ResourceSet,
                human_name="Input subsampled data ResourceSet to check",
                short_description="ResourceSet containing cell culture time series subsampled data",
                optional=False,
            ),
            "metadata_table": InputSpec(
                Table,
    ...
    output_specs: OutputSpecs = OutputSpecs(
        {
            ...
            "filtered_metadata_table": OutputSpec(
                Table,
    ...
    config_specs = ConfigSpecs(
        {
            ...
            "range_checks": ParamSet(
                ConfigSpecs(
                    {
                        "column": StrParam(
                            human_name="Column name", short_description="Column to validate"
                        ),
    ...
    def _check_sample_quality(self, table: Table, params: ConfigParams) -> dict[str, Any]:
        """
        ...
        self.log_info_message(
            f"  Minimum data points checks: {len(min_data_points_checks)} check(s) defined"
        )
        ...
        for check in min_data_points_checks:
            ...
            if non_nan_count < min_count:
                warnings.append(
                    f"{col}: only {non_nan_count} data points (minimum required: {min_count})"
                )
                self.log_warning_message(
                    f"    ⚠️ Insufficient data points: {non_nan_count} < {min_count}"
                )

                if action == "remove_sample":
                    exclude_sample = True
                    self.log_warning_message(
                        "    🚫 Action 'remove_sample' triggered - sample will be EXCLUDED"
                    )
                else:
                    self.log_info_message("    📝 Action 'mark_only' - keeping sample with warning")
            else:
                self.log_info_message(
                    f"    ✅ Sufficient data points: {non_nan_count} >= {min_count}"
                )
        ...
        for check in range_checks:
            ...
            self.log_info_message(
                f"  Checking range for column '{col}': min={min_val}, max={max_val}, action={action}"
            )
            ...
            self.log_info_message(
                f"      - Min: {col_min:.4f}, Max: {col_max:.4f}, Mean: {col_mean:.4f}"
            )
            ...
            if violation_count > 0:
                ...
                self.log_warning_message(
                    f"    ⚠️ {violation_count} values outside range [{min_val}, {max_val}]"
                )
                ...
                if action == "remove_sample":
                    ...
                    df = df[~violations]
                    self.log_info_message(
                        f"    ✂️ Action 'remove_rows' - {violation_count} rows removed"
                    )
                else:
                    self.log_info_message("    📝 Action 'mark_only' - keeping data with warning")
            else:
    ...
    def run(self, params: ConfigParams, inputs) -> dict[str, Any]:
        ...
        metadata_table: Table | None = inputs.get("metadata_table", None)
        ...
        for _resource_name, resource in subsampled_resources.items():
        ...
        for resource_name, resource in data_resources.items():
            ...
            if not quality_result["passed"]:
                ...
                self.log_warning_message(f"Sample '{resource_name}' excluded: {warnings_str}")
                continue
            ...
            if add_quality_tags:
                ...
                if quality_result["warnings"]:
                    output_table.tags.add_tag(
                        Tag(key="quality_warnings", value="; ".join(quality_result["warnings"]))
                    )

                output_table.tags.add_tag(
                    Tag(key="outliers_detected", value=str(quality_result["outliers_detected"]))
                )
                ...
                output_table.tags.add_tag(
                    Tag(key="range_violations", value=str(quality_result["range_violations"]))
                )
        ...
        self.log_info_message(
            f"Filtering subsampled data based on {len(passed_resource_names)} passed samples"
        )
        ...
        for resource_name, resource in subsampled_resources.items():
            ...
            if resource_name in passed_resource_names:
                if not isinstance(resource, Table):
                    self.log_warning_message(
                        f"Skipping non-Table subsampled resource: {resource_name}"
                    )
                    continue
                ...
                if add_quality_tags:
                    subsampled_output_table.tags.add_tag(
                        Tag(key="quality_check_passed", value="true")
                    )
        ...
        if metadata_table is not None:
            ...
            filtered_metadata_df = metadata_df[
                metadata_df[metadata_df.columns[0]].isin(passed_resource_names)
            ]
