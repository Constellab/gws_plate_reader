...
@task_decorator("CellCultureQualityCheck", human_name="Cell Culture Data Quality Check",
    ...
    def _check_sample_quality(
        ...
        for check in min_data_points_checks:
            ...
            if non_nan_count < min_count:
                ...
                self.log_warning_message(f"    ⚠️ Insufficient data points: {non_nan_count} < {min_count}")

                if action == 'remove_sample':
                    exclude_sample = True
                    self.log_warning_message("    🚫 Action 'remove_sample' triggered - sample will be EXCLUDED")
                else:
                    self.log_info_message("    📝 Action 'mark_only' - keeping sample with warning")
            else:
                self.log_info_message(f"    ✅ Sufficient data points: {non_nan_count} >= {min_count}")
