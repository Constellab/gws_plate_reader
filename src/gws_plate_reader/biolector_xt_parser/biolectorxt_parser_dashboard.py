                ...
                for row in range(ROWS):
                    ...
                    for col in range(COLS):
                        ...
                        if well in crossed_out_wells:
                            cols_object[col+1].button(f":gray[{well}]", key=well, help=json.dumps(well_data[well], sort_keys=True, indent=4, ensure_ascii=False), disabled=True)
                        elif well in BiolectorState.get_well_clicked():
                            if cols_object[col+1].button(f":green[{well}]", key=well, help=json.dumps(well_data[well], sort_keys=True, indent=4, ensure_ascii=False)):
                                if well in BiolectorState.get_well_clicked():
                                    BiolectorState.remove_well_clicked(well)
                                has_changed = True
                        elif well in BiolectorState.get_replicated_wells_show():
                            if cols_object[col+1].button(f"**{well}**", key=well, help=json.dumps(well_data[well], sort_keys=True, indent=4, ensure_ascii=False)):
                                BiolectorState.append_well_clicked(well)
                                has_changed = True
                        else:
                            if cols_object[col+1].button(well, key=well, help=json.dumps(well_data[well], sort_keys=True, indent=4, ensure_ascii=False)):
                                BiolectorState.append_well_clicked(well)
                                has_changed = True
